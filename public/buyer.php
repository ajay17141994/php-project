            ...
            <div class="tab-content">
                ...
                <div id="listingtab" class="tab-pane fade in active">
                    ...
                    <?php
                        ...
                        {
                            $lat=$_SESSION["latitude"];//$_GET['lat'];
                            $lon=$_SESSION["longitude"];//$_GET['long'];
                            $userid = $_SESSION["id"];

                            if(isset($lat,$lon))
                            {
                                $R = 6371;// 	radius of earth
                                $rad=5; // radius for product discovery
                                $maxLat = $lat + rad2deg($rad/$R);
                                $minLat = $lat - rad2deg($rad/$R);
                                $maxLon = $lon + rad2deg(asin($rad/$R) / cos(deg2rad($lat)));
                                $minLon = $lon - rad2deg(asin($rad/$R) / cos(deg2rad($lat)));
                                $sql1 = "SELECT DISTINCT pro1.id as pro_id,
                                pro1.latitude AS latitude,
                                pro1.longitude AS longitude,
                                111.1111 * DEGREES(
                                            ACOS(
                                            COS(RADIANS(pro1.latitude)) * COS(RADIANS($lat)) *
                                            COS(RADIANS(pro1.longitude - $lon)) + 
                                            SIN(RADIANS(pro1.latitude)) * SIN(RADIANS($lat))))
                                            AS distance_in_km,
                                        ...
                                        pro1.price as pro_price
                                                ...
                                                where pro1.isActive = 1 AND
                                                (
                                                (pro1.latitude BETWEEN $minLat AND $maxLat) AND 
                                                (pro1.longitude BETWEEN $minLon AND $maxLon) AND
                                                $rad >= 111.1111 * DEGREES(
                                                ACOS(COS(RADIANS(pro1.latitude)) * COS(RADIANS($lat)) * 
                                                COS(RADIANS(pro1.longitude - $lon)) + 
                                                SIN(RADIANS(pro1.latitude)) * SIN(RADIANS($lat)))))";
                            }
                ...
                <?php
                        ...
                        $sql1 = "SELECT ord1.id as ord_id,
                                        ...
                                        FROM products as pro1 
                                                ...
                                                left join subcategories_loc as subloc1 on subloc1.sub_id = pro1.sub_id
                                                left join varieties_loc as varloc1 on varloc1.var_id = pro1.var_id
                                                where ord1.buyer_id = " . $_SESSION["id"] . " AND pro1.isActive = 1 AND ord1.isActive = 1";
